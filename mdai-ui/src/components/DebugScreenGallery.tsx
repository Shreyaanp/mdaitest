...
const PHASE_SEQUENCE: SessionPhase[] = [
  'idle',
  'pairing_request',
  'hello_human',
  'scan_prompt',
...
const PHASE_METADATA: Record<SessionPhase, { label: string; description: string }> = {
...
const MOCK_DATA: Partial<Record<SessionPhase, Record<string, unknown>>> = {
  qr_display: {
...
export default function DebugScreenGallery() {
  ...
  useEffect(() => {
    ...
    send({
      ...
      error: screen.mockData?.error as string | undefined
    })
  }, [send, screens])
  ...
  useEffect(() => {
    ...
    if (currentIndex === 0 && !prevScreen && !isTransitioning) return
  ...
  return (
    <div className="app-shell">
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '10px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '8px' }}>
          <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#4f4' }}>
            🔧 {currentScreen.label}
          </div>
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: '10px' }}>
          <button
            onClick={() => canGoPrevious && !isTransitioning && setCurrentIndex(prev => prev - 1)}
          ...
          <button
            onClick={() => canGoNext && !isTransitioning && setCurrentIndex(prev => prev + 1)}
        ...
        <div style={{ fontSize: '10px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '8px' }}>
