...
export default function QRCodeStage({ token, qrPayload, expiresIn }: QRCodeStageProps) {
  const qrValue = useMemo(
    () => (qrPayload ? JSON.stringify(qrPayload) : token ?? 'waiting_token'),
    [qrPayload, token]
  )
  ...
  return (
    <div className="overlay">
      <div className="overlay-card">
        ...
        {qrPayload && typeof qrPayload.server_host === 'string' ? (
          <p className="token-hint">Bridge: {String(qrPayload.server_host)}</p>
        ) : null}
