...
class RealSenseService:
        ...
        async with self._lock:
            ...
            try:
                ...
                if "frame didn't arrive" in lowered:
                    ...
                    # FIX: Timeouts are usually transient USB/bandwidth issues
                    # Don't restart pipeline aggressively - just skip frames and let it recover
                    if self._consecutive_timeouts <= 30:  # Increased from 8 to 30
                        logger.debug(
                            "RealSense frame timeout (%s consecutive); skipping frame (feed continues)",
                            self._consecutive_timeouts,
                        )
                        return None
                    
                    # Only restart if timeouts persist for >30 frames (~6 seconds at 5fps)
                    # This indicates a real hardware issue, not just transient USB problems
                    logger.warning(
                        "RealSense consecutive timeouts reached %s (>6s); restarting pipeline",
                        self._consecutive_timeouts,
                    )
                    await self._restart_pipeline_unlocked(loop)
                    self._consecutive_timeouts = 0
    ...
    def _serialize_frame(self, result: Optional[LivenessResult]) -> bytes:
        """Generate preview frame - black screen with face dot for privacy/performance."""
        if cv2 is None:
            return self._placeholder_frame()
        
        try:
            # Create black screen with face dot indicator instead of full RGB preview
            # This is MUCH faster and saves bandwidth on Jetson Nano
    ...
    def _create_face_dot_frame(self, result: Optional[LivenessResult]) -> np.ndarray:
        """Create a 640x480 black frame with a green/red dot indicating face detection."""
        # Create black frame (640x480 to match camera resolution)
        frame = np.zeros((480, 640, 3), dtype=np.uint8)
        
        if result is None or result.bbox is None:
            # No face detected - show small red dot in center
            cv2.circle(frame, (320, 240), 8, (0, 0, 255), -1)  # Red dot
            cv2.putText(frame, "No Face", (280, 270), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        else:
            # Face detected - show green dot at face center
            x0, y0, x1, y1 = result.bbox
            center_x = (x0 + x1) // 2
            center_y = (y0 + y1) // 2
            
            # Color based on liveness
            if result.instant_alive:
                color = (0, 255, 0)  # Green - live face
                text = "Live Face Detected"
            else:
                color = (0, 165, 255)  # Orange - face but not live
                text = "Face Detected"
            
            # Draw large dot at face center
            cv2.circle(frame, (center_x, center_y), 15, color, -1)
            cv2.circle(frame, (center_x, center_y), 17, (255, 255, 255), 2)  # White outline
            
            # Optional: Show status text
            cv2.putText(frame, text, (center_x - 80, center_y + 50), 
